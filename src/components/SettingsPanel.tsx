...
interface Props {
  ...
  onUpdateProfile: (updates: { avatarUrl?: string | null; avatarIsGif?: boolean; bannerUrl?: string | null; bannerIsGif?: boolean; bio?: string }) => Promise<{ success: boolean }>;
  onLogout: () => void;
  onClose: () => void;
}
...
export default function SettingsPanel({ user, onChangeName, onUpdateProfile, onLogout, onClose }: Props) {
  ...
  const handleSaveProfile = async () => {
    ...
    setProfileSuccess('');
    // avatarIsGif: önizleme data:image/gif ise VEYA mevcut URL'si https:// ve avatarIsGif true ise
    const avatarIsGif = !!(avatarPreview?.startsWith('data:image/gif') || (user.avatarIsGif && avatarPreview?.startsWith('https://')));
    const bannerIsGif = !!(bannerPreview?.startsWith('data:image/gif') || (user.bannerIsGif && bannerPreview?.startsWith('https://')));
    await onUpdateProfile({ avatarUrl: avatarPreview, avatarIsGif, bannerUrl: bannerPreview, bannerIsGif, bio });
        ...
        <div className="flex-1 p-8 overflow-y-auto">
          {activeTab === 'profile' && (
            <div>
              ...
              >{profileSaving ? '⏳ GIF yükleniyor...' : 'Değişiklikleri Kaydet'}</button>
            </div>
          )}
