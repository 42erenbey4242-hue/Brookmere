    ...
    <div className="w-60 bg-[#2b2d31] flex flex-col shrink-0 overflow-y-auto py-4 px-2">
      ...
      {contextMenu && (
        ...
        >
          ...
          <button
            onClick={() => { setShowRenameInput(v => !v); setRenameValue(contextMenu.user.username); setRenameError(''); setShowTimeoutInput(false); }}
          ...
          {showRenameInput && (
            <div className="px-3 py-2 space-y-2">
              <input
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter') {
                    const result = onRenameUser(contextMenu.user.id, renameValue);
                    if (result.success) { setContextMenu(null); setShowRenameInput(false); }
                    else setRenameError(result.error || 'Hata.');
                  }
                }}
                autoFocus
              />
