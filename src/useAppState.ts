...
import {
  ...
  fbDeleteServer, fbDeleteUser, fbWriteDeviceCooldown, uploadProfileImage,
} from './store';
...
export function useAppState() {
  ...
  // Admin herhangi bir kullanıcının adını değiştirebilir (cooldown yok)
  const adminRenameUser = useCallback((userId: string, newName: string): { success: boolean; error?: string } => {
    const s = stateRef.current;
    const user = s.users[userId];
    if (!user) return { success: false, error: 'Kullanıcı bulunamadı.' };
    if (!newName.trim() || newName.trim().length < 2) return { success: false, error: 'İsim en az 2 karakter olmalıdır.' };
    if (newName.trim().length > 32) return { success: false, error: 'İsim en fazla 32 karakter olabilir.' };
  ...
  const updateProfile = useCallback(async (userId: string, updates: { avatarUrl?: string | null; avatarIsGif?: boolean; bannerUrl?: string | null; bannerIsGif?: boolean; bio?: string }): Promise<{ success: boolean }> => {
    const s = stateRef.current;
    const user = s.users[userId];
    ...
    // GIF'leri Firebase Storage'a yükle — base64 data URL ise yükle, URL döner
    try {
      if (finalUpdates.avatarIsGif && finalUpdates.avatarUrl?.startsWith('data:')) {
        finalUpdates.avatarUrl = await uploadProfileImage(userId, 'avatar', finalUpdates.avatarUrl);
      }
      if (finalUpdates.bannerIsGif && finalUpdates.bannerUrl?.startsWith('data:')) {
        finalUpdates.bannerUrl = await uploadProfileImage(userId, 'banner', finalUpdates.bannerUrl);
      }
    } catch (e) {
      console.warn('GIF upload failed:', e);
      // Yükleme başarısız olsa bile base64 ile devam et (lokal görünür)
    }

    const updated = { ...user, ...finalUpdates };
    const next = { ...s, users: { ...s.users, [userId]: updated } };
    stateRef.current = next; saveLocal(next); setStateRaw(next);
    fbWriteUser(updated);
    return { success: true };
  ...
  return {
    ...
    changeName, adminRenameUser, updateProfile, banUser, timeoutUser, removeTimeout,
